...
{
    NSLog(@"[%@]-->showEmpty:{key:%@,section:%d}",[self class],model.key,model.sectionNumber);
...
{
    NSLog(@"[%@]-->showLoading:{key:%@,section:%d}",[self class],model.key,model.sectionNumber);
...
{
    NSLog(@"[%@]-->showModel:{key:%@,section:%d}",[self class],model.key,model.sectionNumber);
...
{
    NSLog(@"[%@]-->showError:{key:%@,section:%d}",[self class], model.key,model.sectionNumber);
...
- (void)reloadModelByKey:(NSString*)targetKey
{
    [_modelDictInternal enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        if ([key isEqualToString : targetKey]) {
            ...
            [self.dataSource removeItemsForSection:model.sectionNumber];
            [self reloadTableView];
            [model reload];
        }
    }];
...
{
    NSLog(@"[%@]-->showComplete:{section:%d}",[self class],model.sectionNumber);
