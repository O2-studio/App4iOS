...
- (void)initRequestWithBaseURL:(NSString *)url
{
     ...
     */
    if (self.usePost) {
        self.mtopRequest.timeOutSeconds = 15.0f;
    }
    else
...
- (void)requestDidLoadFailed:(MTOPRequest *)request{
    ...
    if (!err) {
        err = [NSError errorWithDomain:TBCitySBMVCErrorDomain code:kRequestTimeout userInfo:@{NSLocalizedDescriptionKey : @"小二很忙，系统很累，请稍后重试"}];
    }
